...
// PATCHED (see gameserver_extra.odin): Valve packs callbacks with pack(4) on
// Linux/macOS and pack(8) on Windows; upstream's `true` default misreads any
// callback struct whose layout differs (e.g. ValidateAuthTicketResponse
// .OwnerSteamID) on Linux.
CALLBACK_PACK_LARGE :: #config(STEAMWORKS_CALLBACK_PACK_LARGE, ODIN_OS == .Windows)
